...
class Qtuple:
    def __init__(self, index, op, arg1, arg2, res):
        self.id, self.op, self.arg1, self.arg2, self.res = index, op, arg1, arg2, res
...
def genqs(lines):
    ...
    def var_def(token):
        ...
        token = next(it)
        # 类型部分开始
        if token.code not in {reservedWordDict['integer'], reservedWordDict['bool'], reservedWordDict['char']}:
    ...
    def assign(token):
        ...
        if token.code != Identifier_Code:
            ...
            if token.context not in varDict.keys():
    ...
    def bool_fac(token):
        ...
        elif token.code == reservedWordDict['true']:
        ...
        elif token.code == Identifier_Code:
            trueEx=len(qTupleList)
            if token.context not in varDict.keys():
                raise gramException(token.row_i, "未定义的标识符")
            gen('jnz', token.context, None, 0)
    ...
    def while_stm(token):
        ...
        backpatch(stm_quit, code_begin)
        gen('j', None, None, code_begin)
